...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes.length - 1; i += 1) {
    result = getCommonPath(pathes[i], pathes[i + 1]);
  }
...
function evaluateTicTacToePosition(position) {
  const result = []; const arr1 = []; const arr2 = [];
  function check(arr) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    arr1.push(position[i][i]);
    arr2.push(position[i][position.length - 1 - i]);
  }
  result.push(check(arr1));
  result.push(check(arr2));
